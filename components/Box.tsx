...
export const BoxContainer: React.FC<React.HTMLAttributes<HTMLDivElement> & {size: Size, mobilesize?: Size}> = (props) => {
  const {size, mobilesize, children, ...slimProps} = props;
  return (
    <>
      <Hidden on="pc">
...
export const Box: React.FC<React.HTMLAttributes<HTMLDivElement> & {size: Size, mobilesize?: Size, mode?: 'normal' | 'flex' | 'center' | 'fluid' | 'flex-fluid'| 'center-fluid'}> = (props) => {
  const {size, mobilesize, children, mode, className, ...slimProps} = props;
    ...
    <>
      <Hidden on="pc">{
        (() => {
          ...
          return (
            <div {...slimProps} className={cls(classes.box, modeClass, classes['x' + _size + (['boxFluidCenter', 'boxFluidFlex', 'boxFluid'].includes(modeStr) ? 'Fluid': '')], className)} >
              {children}
            </div>
          );
        })()
      ...
      <Hidden on="mobile">{
        (() => {
          ...
          return (
            <div {...slimProps} className={cls(classes.box, modeClass, classes['x' + size + (['boxFluidCenter', 'boxFluidFlex', 'boxFluid'].includes(modeStr) ? 'Fluid': '')], className)} >
              {children}
            </div>
          );
        })()
